// @ts-nocheck — near-verbatim port of the ThreeUI SylvaHero reference runtime
/* ====================================================================== *
 ...
 *
 * Ported from the ThreeUI SylvaHero reference into an ES module: THREE comes
 * from npm, every listener/rAF/timeout is tracked, and the ready/intro state
 * classes live on the hero root element instead of <body> so the whole thing
 * can mount and unmount with the route.
 * ====================================================================== */
import * as THREE from "three";

export function mountHeroRuntime(rootEl: HTMLElement): { dispose(): void } {
  'use strict';
  ...
  function startTick() {
    if (ticking) return;
    ticking = true;
    (function loop() { if (disposed) return; requestAnimationFrame(loop); tick(); })();
  }
  ...
  function startParallax() {
    ...
    parOn = true;
    var nodes = rootEl.querySelectorAll(PARALLAX);
    ...
    on(window, 'pointerleave', function () {
      pointer.x = pointer.y = 0; ndc.x = 10;
    });
  }

  var canvas   = rootEl.querySelector('#scene') as HTMLCanvasElement;
  var hero     = rootEl;
  var stageEl  = rootEl.querySelector('#stage') as HTMLElement;
  ...
  function startPortalReveal() {
    ...
    portalStarted = true;
    var figs = rootEl.querySelectorAll('.portal');
  ...
  function initDock() {
    ...
    if (document.fonts && document.fonts.ready) document.fonts.ready.then(function () { if (!disposed) measureDock(); });
    on(window, 'resize', measureDock);
    ...
    /* the pill throws a handful of pollen on click — navigation itself is
       react-router's business, so no preventDefault here */
  ...
  function ready() {
    ...
    later(function () { rootEl.classList.add('intro-done'); }, REDUCED ? 0 : 2900);
  }
  ...
  return {
    dispose: function () {
      if (disposed) return;
      disposed = true;
      for (var i = 0; i < cleanups.length; i++) { try { cleanups[i](); } catch (e) { /* already gone */ } }
      cleanups.length = 0;
      if (scene) {
