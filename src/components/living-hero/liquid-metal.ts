...
/* Ported from the ThreeUI SylvaHero reference: mounts one liquid-metal
   control on a [data-liquid-metal] host. Returns a disposer, or null when
   WebGL2 is unavailable (the plain styled button stays usable). */
export function mountLiquidMetal(host: HTMLElement): { dispose(): void } | null {
    ...
    const window = {
      ...
      addEventListener(type, fn, opts) {
        hostWindow.addEventListener(type, fn, opts);
        cleanups.push(() => hostWindow.removeEventListener(type, fn, opts));
      }
    };
...
if(!gl){
  /* no WebGL2: leave the DOM button as-is and mount nothing */
  return null;
} else {
...
return {
  dispose() {
    if (disposed) return;
    disposed = true;
    for (const fn of cleanups) { try { fn(); } catch (e) { /* already gone */ } }
    cleanups.length = 0;
